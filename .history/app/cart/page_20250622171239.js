...
export default function CartPage() {
  ...
  // رقم المستخدم ثابت 7 (يمكن تغييره أو استخدام localStorage)
  const userId = "7";
  ...
  const fetchCartItems = async (userId) => {
    try {
      ...
      // تأكد من بنية البيانات
      // مثلاً: { items: [...] } أو { data: [...] } حسب الـ API
      ...
      {cartItems.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
          {cartItems.map((item) => (
            ...
            >
              <div style={{ flex: "1" }}>
                <h3 style={{ margin: 0 }}>{item.nameproducts || item.product_name}</h3>
                <p style={{ margin: "5px 0", color: "#666" }}>
                  الكمية: {item.quantity}
                </p>
                <p style={{ margin: "5px 0", color: "#222" }}>
                  السعر الإجمالي: {(item.total_amount || (item.price * item.quantity))} ر.س
                </p>
              </div>
              <img
                src={item.image_url}
                alt={item.nameproducts || item.product_name}
                style={{
                  width: "100px",
                  height: "100px",
                  objectFit: "cover",
