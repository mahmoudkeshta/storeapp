...
export default function PostPage({ params }) {
  ...
  const [likedProducts, setLikedProducts] = useState({}); // حالة الإعجاب لكل منتج

  // معرف المستخدم - استبدل هذا بالمعرف الحقيقي للمستخدم
  const storedUserId = typeof window !== "undefined" ? localStorage.getItem("userId") : null;
  const userId = storedUserId || "8"; // قيمه افتراضية لو غير موجود
  ...
  // إرسال المنتج إلى السلة في السيرفر
  ...
  // دالة تبديل حالة الإعجاب (تفعيل/تعطيل)
          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              >
                ...
                {/* زر الإعجاب صغير تحت الصورة يمين */}
                <div style={{ display: "flex", justifyContent: "flex-end", paddingRight: "5px", marginBottom: "8px" }}>
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      fontSize: "22px",
                      color: likedProducts[product.product_id] ? "#e53935" : "#ccc",
                      transition: "color 0.3s ease",
                      padding: 0,
                      lineHeight: 1,
                    }}
                    aria-label={likedProducts[product.product_id] ? "معجب" : "غير معجب"}
                  >
                    {likedProducts[product.product_id] ? "❤️" : "🤍"}
                  </button>
                </div>
