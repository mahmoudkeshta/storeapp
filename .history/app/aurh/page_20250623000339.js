...
export default function Auth() {
  ...
  // تحقق من وجود مستخدم مسجل دخول عند تحميل الصفحة
  useEffect(() => {
    const user = localStorage.getItem("user");
    if (user) {
      router.push("/category"); // إذا مسجل دخول، وجهه للداشبورد
    }
  }, [router]);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        alert(`مرحباً، ${data.data.username}!`);
        ...
        localStorage.setItem("username", data.data.username.toString());
        // توجه للداشبورد بعد تسجيل الدخول
